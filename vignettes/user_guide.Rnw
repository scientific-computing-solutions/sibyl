...
The KM curve and KM survival estimates are always performed without covariates or strata variables (only intercept), whereas the Cox model is fitted twice, once using the covariates and strata variables if entered as inputs and once unstratified.
<<>>=
...
To help decide which spline model to include we can use the \texttt{createSplineAICTable} function to fit multiple spline models simultaneously:

<<>>=
splineAICTable <- createSplineAICTable(models.withArm, 
                     k=2:4,
...
If \texttt{class} is set to FlexTable then a FlexTable object is output ready for inclusion in a reprteRs Word document.  
...
\item seed: Random seed (only used when covariates are used and curves must be simulated)
