...
\section{Data}
The \texttt{Sibyl} package contains an example dataset which is derived from a dataset in the \texttt{asaur} package. 
<<>>=
  data( sibylData )
@
...
In order for \texttt{Sibyl} to correctly output information about covariates, treatment arm and subgroups, we first need to define these using the \texttt{ColumnDef} function. Note: all must be defined otherwise an error will be shown when creating the \texttt{SurvivalData object} (Section~\ref{sec:survdata}). Further, \texttt{ColumnDef} it is recommended that the name of the column is as it appears in the raw data as this simplifies when creating the \texttt{SurvivalData} object. The information needed to define the columns are listed below:
...
First we define the columns corresponding to treatment arm in the order where control is listed first then active. 
<<>>=
armDef <- ColumnDef( columnName = "grp",
                     type = "categorical",
                     categories = c("patchOnly", "combination") )
@
This gives a message that categories have been specified for a categorical variable, but not as a factor. The danger of defining them as a vector rather than a factor is that their order might not be preserved through all the package functionality. The message makes explicit what order has been assumed for the categories.

If we provide categories as a factor, or define a non-categorical column, then this warning does not appear which we is shown below where we define the covariates.
<<>>=
covariateDef <- list(
...
@
Finally we define the subgroups. Note the displayNames that will be used in tabular output. 
<<>>=
subgroupDef <- list(
...
\label{sec:survdata}
The next step will be to create the \texttt{SurvivalData} object which is the basis for any downstream outputs, plots and analyses. 
<<>>=
...
The first input is the raw data as previously described. The next three define which columns represent the definition we created for treatment arm, covariates and subgroups respectively. Several time-to-event variables can be added at the same time. Important is that the length and order of these three columns match (endPointNames, censorCol, timeCol). The first endpoint name is associated with the first censor column and the first time column etc. 
\begin{itemize}
\item subjectCol: the name of the column that defines the subject ID
\item endPointNames: a vector of strings; each gives the name of a different end-point of interest
\item censorCol: a vector of strings; indicator columns for whether the subject was censored for each end-point
\item timeCol: a vector of strings; time of event columns for each end-point.
\end{itemize}
...
\item Restricted Mean survival time (stratified or unstratified)
\end{enumerate}
...
The KM curve and KM survival estimates are always performed without covariates or strata variables (only intercept), whereas the Cox model and RMST use the covariates and strata if entered as inputs.
<<>>=
sP <- fitSemiParametric(my.data,endPoint="relapse",covariates=c("age"),strata=c("race"))
sP2 <- fitSemiParametric(my.data,endPoint="newEndpoint",subgroup="sub.isMale")
@

The fitSemiParametric function creates a \texttt{SemiParametricModel} object. Note if a subgroup has been used then only the appropriate subset of the data is found inside the SemiParametricModel object:
<<>>=
...
<<>>=
plot(sP, type="KM", n.at.risk=TRUE, pch=NA, col=1:2, conf.int=FALSE)
plot(sP, type="CumHaz", use.facet=FALSE,logTime=FALSE)
plot(sP, type="loglogs", use.facet=TRUE)
@
...
and plotted:
<<>>=
plot(coxSnellres$time, -log(coxSnellres$surv), type='s', xlab="Cox-Snell Residuals", ylab="Estimated Cumulative Hazard Function")
...
rmstValue <- calcRmst(sP, alpha=0.05, trunc=40, class="rmst")
...
\subsection{Fitting}
The fitting of models is rather flexibel and the package makes it easy to fit the models, using the \texttt{fitModels} function, under the following options: 
\begin{enumerate}
...
<<>>=
models.withArm <- addModel(models.withArm, c("gengamma", "spline"), 
                           modelOptions=list(spline=list(k=3,scale="odds")))
names(models.withArm@models)
@

Note it is possible to pass arguments to the model fitting functions using the modelOptions argument (both when adding a model and when initially creating the survival model object). This should be a names list of lists. In the example above the arguments \texttt{k=3} and \texttt{scale="odds"} are to be passed to the spline model fitting function. 

We can also remove models, if for example the fit is poor and we want to exclude the model from any further analyses/output:
<<>>=
models.withArm <- removeModel(models.withArm, c("gengamma", "spline"))
names(models.withArm@models)
@
Note, as before, if a subgroup has been used then only the appropriate subset of the data is found inside the SurvivalModel object.
...
<<>>=
coeffWithArm <- summariseCoeffs(models.withArm, class="matrix")
coeffByArm <- summariseCoeffs(models.byArm, class="matrix")
...
<<>>=
covarsWithArm <- vcov(models.withArm, class="matrix")
choleskyDecompositions <- getCholeskyDecomp(models.withArm,class="matrix")
...
\end{equation}
where $L$ is the likelihood, $n$ number of observations and $k$ the number of free model parameters. Again setting the class argument to FlexTable (default) will output a sorted, formatted table to be included in a word document, alternatively a data frame can be output. 
<<>>=
icByArm <- createIcTable(models.byArm, class="data.frame")
print(icByArm)
@
...
<<>>=
lifeTableByArm <- createLifeTable(models.byArm, times = seq(0, 20, 4), class="data.frame")
...
\item Nsim: the number of simulations to use when creating the average survival curves
\end{itemize}

The simulation step takes a bit of time, so please be patient.
<<>>=
#Calculate values of KM and parametric model curves
paramSurvCurveByArm <- createAvCurvePlotData(models.byArm, Npoints=151)
paramSurvCurveWithArm <- createAvCurvePlotData(models.withArm, maxTime=100, models="weibull")
@
...
plot(paramSurvCurveByArm)
plot(paramSurvCurveWithArm)
@
