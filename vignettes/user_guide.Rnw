...
To change the reference arm, instead of defining levels, you can move the factor order around to switch the reference arm i.e. categories = c("combination","patchOnly")).
...
@
The KM option displays slightly differently from the other options because it uses a wrapper around \texttt{plot.survfit} rather than \texttt{ggplot2}. The extra arguments given make its appearance similar to the others'. See \texttt{help(kmPlotWrapper)} for further details.
